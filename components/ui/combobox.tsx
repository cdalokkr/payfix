    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        ...
        >
          ...
          <ChevronsUpDown className="h-3.5 w-3.5 shrink-0 opacity-50 text-slate-400" />
        </button>
      </PopoverTrigger>
      <PopoverContent align="start" className="w-[var(--radix-popover-trigger-width)] min-w-[200px] p-1.5 bg-white border border-gray-200 rounded-[14px] shadow-xl z-50">
        {options.length > 5 && (
          <div className="flex items-center border-b border-gray-100 px-2.5 pb-1.5 pt-1">
        ...
        <div className="max-h-48 overflow-y-auto py-1 space-y-0.5">
          {filteredOptions.length === 0 ? (
            <p className="p-2 text-center text-xs text-slate-400 font-medium">{emptyText}</p>
          ) : (
            filteredOptions.map((option) => (
              ...
              >
                ...
                {value === option.value && (
                  <Check className="h-3.5 w-3.5 text-indigo-600 shrink-0 ml-1.5" />
                )}
              </button>
            ))
          )}
